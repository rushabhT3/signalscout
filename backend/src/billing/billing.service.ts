...
export class BillingService {
  ...
  private async onCheckoutCompleted(session: Stripe.Checkout.Session): Promise<void> {
    ...
    if (!userId) {
      this.logger.warn("checkout.session.completed without client_reference_id");
      return;
    }
    ...
    if (!wasAlreadyPro) {
      await this.credits.grant(userId, PLAN_LIMITS.pro.monthlyCredits, "plan_upgrade", session.id);
  ...
  private async syncSubscription(subscription: Stripe.Subscription): Promise<void> {
    ...
    if (!userId) {
      this.logger.warn(`Subscription ${subscription.id} could not be matched to a user.`);
      return;
    }
    ...
    this.logger.log(`Synced subscription for ${userId}: ${subscription.status}`);
  }
