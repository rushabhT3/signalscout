...
export class MockEvaluator implements AiEvaluator {
  ...
  evaluateSignal(input: EvaluateSignalInput): Promise<SignalEvaluation> {
    const haystack =
      `${input.posting.title} ${input.posting.description}`.toLowerCase();
    const matchedKeywords = input.keywords.filter((keyword) =>
      haystack.includes(keyword.toLowerCase()),
    );
    const titleLooksGtm =
      /(sales|account executive|sdr|revenue|growth|gtm|marketing)/i.test(
        input.posting.title,
      );
    ...
    const confidence = isMatch
      ? Math.min(
          95,
          45 + matchedKeywords.length * 20 + (titleLooksGtm ? 15 : 0),
        )
      : 12;
      ...
      reasoning: isMatch
        ? `The posting "${input.posting.title}" aligns with the hypothesis${
            matchedKeywords.length > 0
              ? ` via keywords: ${matchedKeywords.join(', ')}`
              : ''
          }.`
