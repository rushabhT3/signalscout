...
export class SignalsController {
  ...
  @ApiOperation({ summary: "Generate an AI outreach draft for a matched signal" })
  generateOutreach(
    @CurrentUser() user: AuthenticatedUser,
    @Param("id", ParseUUIDPipe) id: string,
    @Body(new ZodValidationPipe(generateOutreachSchema)) input: GenerateOutreachInput,
  ): Promise<SignalView> {
    return this.signals.generateOutreach(user.id, id, input);
  }

  @Post("trackers/:trackerId/run")
  @ApiOperation({ summary: "Run a tracker: ingest postings and evaluate new ones" })
  run(
    @CurrentUser() user: AuthenticatedUser,
